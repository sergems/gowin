...
export async function autoSettleFinishedFixtures(): Promise<{
  ...
  if (selections.length === 0) return { settled: 0, won: 0, lost: 0 };
  ...
  for (const bet of pendingBets) {
    ...
    const outcomes = betSelections.map((s) => {
      const fixture = fixtureMap[s.fixtureId];
      if (!fixture || fixture.scoreHome === null || fixture.scoreAway === null) return null;
      return getSelectionOutcome(s.selection, s.market, fixture.scoreHome!, fixture.scoreAway!);
    });

    if (outcomes.some((o) => o === null)) continue;

    const allWon = outcomes.every((o) => o === true);
