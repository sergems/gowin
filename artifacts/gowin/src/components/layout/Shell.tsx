...
export function Shell({ children }: { children: ReactNode }) {
  ...
  const adminLinks = user?.role === "admin" ? [
    ...
    { href: "/admin/branch-performance", icon: BarChart3,         label: t("nav.branch_performance"),   match: (l: string) => l === "/admin/branch-performance" },
    { href: "/admin/fixtures",           icon: Activity,          label: t("nav.fixtures"),             match: (l: string) => l === "/admin/fixtures" },
    { href: "/admin/bets",               icon: Settings,          label: t("nav.bet_management"),       match: (l: string) => l === "/admin/bets" },
          ...
          <nav className={`space-y-1 ${open ? "px-2" : "px-1"}`}>
            ...
            {!isStaffRole && (
              <div
                ...
                className={`pointer-events-none select-none flex items-center gap-3 rounded-md text-sm font-medium opacity-35
                  ${open ? "px-3 py-2" : "px-0 py-2 justify-center"}`}>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative min-w-0">
        ...
        <footer className="shrink-0 border-t border-border bg-card/50 px-4 py-2 hidden md:block">
          <div className="flex items-center justify-center gap-3 text-[10px] text-muted-foreground/50">
            ...
            <span>·</span>
            <a href="/responsible-gambling" className="hover:text-muted-foreground transition-colors">{t("footer.responsible_gambling")}</a>
          </div>
        </footer>
      </main>
